                ...
                if self.categories and not self.from_date and not self.to_date:
                    ...
                    for catg in self.categories:
                        if office_connector.calendar_id and office_connector.calendar_id.calendar_id:
                            url = "https://graph.microsoft.com/v1.0/me/calendars/"+str(office_connector.calendar_id.calendar_id)+"/events?$filter=categories/any(a:a+eq+'{}')".format(
                                catg.name.replace(' ', '+'))
                        else:
                    ...
                    if self.from_date and self.to_date:
                        if office_connector.calendar_id and office_connector.calendar_id.calendar_id:
                            url = 'https://graph.microsoft.com/v1.0/me/calendars/'+str(office_connector.calendar_id.calendar_id)+'/events?$filter=lastModifiedDateTime%20gt%20{}%20and%20lastModifiedDateTime%20lt%20{}' \
                        ...
                        if self.from_date and self.to_date:
                            if office_connector.calendar_id and office_connector.calendar_id.calendar_id:
                                url = "https://graph.microsoft.com/v1.0/me/calendars/" + str(
                                    ...
                                    self.from_date.strftime("%Y-%m-%dT%H:%M:%SZ"), self.to_date.strftime("%Y-%m-%dT%H:%M:%SZ")) + " and categories/any(a:a+eq+'{}')".format(catg.name.replace(' ', '+'))
                            else:
            ...
            while True:
                ...
                for event in events:
                    ...
                    if odoo_meeting:
                            ...
                            odoo_meeting.write({
                                ...
                                'location': (event['location']['address']['city'] + ', ' + event['location']['address'][
                                    'countryOrRegion']) if 'address' in event['location'] and 'city' in
                                                           event['location'][
                                                               'address'].keys() else "",
                                'start': datetime.strptime(datetime.strftime(p.parse(event['start']['dateTime']), "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%dT%H:%M:%S"),
                                'stop': datetime.strptime(datetime.strftime(p.parse(event['end']['dateTime']), "%Y-%m-%dT%H:%M:%S"),"%Y-%m-%dT%H:%M:%S"),
                                'allday': event['isAllDay'],
                                'categ_ids': [(6, 0, categ_id)] if categ_id else None,
                                'show_as': event['showAs'] if 'showAs' in event and (
                                        event['showAs'] == 'free' or event['showAs'] == 'busy' or event[
                                    'showAs'] == 'tentative') else None,
                        ...
                        if event['recurrence']:
                            if event['recurrence']['range']['type'] == 'noEnd':
                                final_date = datetime.strptime(event['recurrence']['range']['startDate'],'%Y-%m-%d').strftime('%Y-%m-%d')
                            else:
                                final_date = datetime.strptime(event['recurrence']['range']['endDate'],'%Y-%m-%d').strftime('%Y-%m-%d')
                        else:
                            final_date = None
                ...
                for meeting in meetings:
                    ...
                    if meeting.recurrency:
                        ...
                        if meeting.month_by == 'date':
                            ...
                            event_type = 'absoluteMonthly'
                            day = meeting.recurrence_id.day
                            if meeting.recurrence_id.byday == '1':
                                week_index = 'first'
                            elif meeting.recurrence_id.byday == '2':
                                week_index = 'second'
                            elif meeting.recurrence_id.byday == '3':
                                week_index = 'third'
                            elif meeting.recurrence_id.byday == '4':
                                week_index = 'fourth'
                            elif meeting.recurrence_id.byday == '-1':
                                week_index = 'last'
